			...
			<div class="card-body py-3">
				...
				@if($tasks->isNotEmpty())
					...
					<table class="table table-sm table-condensed table-row-gray-100 align-middle gs-0 gy-3 table-row-bordered">
						...
						<form action="{{ route('task-status-change', $task->id) }}" method="POST" style="display: inline;">
							@csrf
							@method('PUT')
						<tr>

							<td class="ps-5 text-dark fs-6">{{($tasks->currentPage() - 1) * $tasks->perPage() + $loop->iteration}}</td>
							<td class="text-dark fs-6">{{ $task->task_name }}</td>
							@if(Auth::user()->role_id == config('constants.ADMIN_ROLE_ID'))
							<td class="text-dark fs-6">{{ $task->first_name }} {{ $task->last_name }}</td>
							@endif
							<td class="text-dark fs-6">{{ $task->description }}</td>
							<td class="text-dark fs-6">{{ Carbon::parse($task->due_date)->format('d-m-Y') }}</td>
		                    <td>
								<select class=" form-control form-control-sm form-control-solid" id="assigned_to" name="status"
							...
							<td class="text-end">
								...
								<a href="{{ route('sms-template-delete', $task->id) }}"
